...
static int allocate_buffers(struct subsys_backup *backup_dev)
{
	...
	backup_dev->img_buf.vaddr = dma_alloc_coherent(backup_dev->dev,
	...
	backup_dev->scratch_buf.vaddr = dma_alloc_coherent(backup_dev->dev,
...
buf_size_err:
	devm_kfree(backup_dev->dev, backup_dev);
	return ret;
}
